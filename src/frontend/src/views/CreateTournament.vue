    ...
    <div class="create">
        ...
        <div v-for="(name, index) in form.stage_name" :key="'group' + index">
            ...
            <label
                ...
                <input
                    type="number"
                    v-model.number="form.number_of_teams[index]"
                    min="1"
                />
            </label>
        </div>
        ...
        <div v-for="(pause, index) in form.break_length" :key="'pause' + index">
            <label
                >Dauer der Pause {{ index + 1 }} (Minuten):
                <input
                    type="number"
                    v-model.number="form.break_length[index]"
                    min="1"
                />
            </label>

            <label
                >Beginn der Pause {{ index + 1 }}:
...
export default {
    ...
    methods: {
        updateStages() {
            this.form.stage_name = Array(this.form.number_of_stages)
                .fill("")
                .map((_, i) => `Gruppe ${String.fromCharCode(65 + i)}`);
            this.form.number_of_teams = Array(this.form.number_of_stages).fill(
                4
            );
        },
        updateBreaks() {
            this.form.break_length = Array(this.form.number_of_breaks).fill(5);
            this.form.break_times = Array(this.form.number_of_breaks).fill(
                "10:00"
            );
        },
        submit() {
